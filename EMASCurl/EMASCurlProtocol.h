...
// 设置支持的HTTP版本，默认HTTP2（全局设置，推荐使用配置对象）
// 更高版本一定包含低版本
// HTTP3需要特殊的编译方式支持，且会引入更大的包体积，参考完整的readme文档
+ (void)setHTTPVersion:(HTTPVersion)version;

// 设置是否开启内部Gzip压缩（全局设置，推荐使用配置对象）
// 开启后，请求的header中会自动添加`Accept-Encoding: deflate, gzip`，并自动解压
// 默认开启，若关闭，则依赖gzip能力时，需要自行处理请求/响应中的gzip字段
+ (void)setBuiltInGzipEnabled:(BOOL)enabled;

// 设置CA证书文件路径（全局设置，推荐使用配置对象）
// 在使用自签名证书做测试时使用
+ (void)setSelfSignedCAFilePath:(nonnull NSString *)selfSignedCAFilePath;

// 是否开启内部重定向支持（全局设置，推荐使用配置对象）
+ (void)setBuiltInRedirectionEnabled:(BOOL)enabled;

// 设置是否开启调试日志（全局设置，推荐使用配置对象）
+ (void)setDebugLogEnabled:(BOOL)debugLogEnabled;

#pragma mark - 日志相关方法

// 设置全局日志级别（全局设置，推荐使用配置对象）
...
// 设置拦截域名黑名单（全局设置，推荐使用配置对象）
// 处理请求时，先检查黑名单，再检查白名单，不拦截黑名单中的域名，传入nil时清除黑名单
+ (void)setHijackDomainBlackList:(nullable NSArray<NSString *> *)domainBlackList;

// 设置用于公钥固定(Public Key Pinning)的公钥文件路径（全局设置，推荐使用配置对象）
// libcurl 会使用此文件中的公钥信息来验证服务器证书链中的公钥，传入nil时清除公钥固定设置
//
// 要求公钥 PEM 文件的结构：
...
// 设置是否开启证书校验（全局设置，推荐使用配置对象），默认开启
+ (void)setCertificateValidationEnabled:(BOOL)enabled;

// 设置是否开启域名校验（全局设置，推荐使用配置对象），默认开启
+ (void)setDomainNameVerificationEnabled:(BOOL)enabled;

// 设置手动代理服务器（全局设置，推荐使用配置对象）
// 设置后会覆盖系统代理设置，传入nil时恢复使用系统代理设置
...
// 设置是否启用HTTP缓存（全局设置，推荐使用配置对象），默认启用
